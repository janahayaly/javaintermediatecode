...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        int average = sum / (3 * bmp.length * bmp[0].length);

        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                res[row][col] = new Pixel(
                (int) Math.round(multiplier * (p.getRed() - average)) + average, 
                (int) Math.round(multiplier * (p.getGreen() - average)) + average, 
                (int) Math.round(multiplier * (p.getBlue() - average)) + average);
            }
        }
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                for (int i = 0; i < numColors; i++) {
                    int dist = bmp[row][col].distance(map[i]);
                    if (dist < minDist) {
                        minDist = dist;
                        sim = map[i];
                    }
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                res[row][col] = new Pixel((int) Math.round(rSum / pixCount), 
                (int) Math.round(gSum / pixCount), (int) Math.round(bSum / pixCount));
            }
        }
        return new PixelPicture(res);
    }
