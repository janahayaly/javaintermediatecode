...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        Pixel[][] og = pic.getBitmap();
        Pixel[][] res = new Pixel[h][w];

        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                if (row < borderWidth || col < borderWidth) {
                    res[row][col] = borderColor;
                } else if (row > h - borderWidth - 1 || col > w - borderWidth - 1) {
                    res[row][col] = borderColor;
                } else {
                    res[row][col] = og[row - borderWidth][col - borderWidth];
                }
            }
        }
    ...
    public static PixelPicture scaleColors(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int g = p.getGreen();
                int b = p.getBlue();
                bmp[row][col] = new Pixel((int) Math.round(rfactor * r), 
                (int) Math.round(gfactor * g), (int) Math.round(bfactor * b));
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        Pixel[][] bmp = pic.getBitmap();
        Pixel[][] oth = f.getBitmap();

        for (int col = 0; col < w1; col++) {
            for (int row = 0; row < h1; row++) {
                Pixel p1 = bmp[row][col];
                Pixel p2 = oth[row][col];
                int r1 = p1.getRed();
                int r2 = p2.getRed();
                int g1 = p1.getGreen();
                int g2 = p2.getGreen();
                int b1 = p1.getBlue();
                int b2 = p2.getBlue();
                bmp[row][col] = new Pixel(weightedAverage(alpha, r1, r2), 
                weightedAverage(alpha, g1, g2), weightedAverage(alpha, b1, b2));
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static void main(String[] args) {
        PixelPicture pic = new PixelPicture("Italy.png");

        PixelPicture newPic = rotateCCW(pic);
    }
}
